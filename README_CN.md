...
- [ACP 集成（通过 acpx 调用编程 Agent）](#acp-集成通过-acpx-调用编程-agent)
- [许可证](#许可证)
...
## ACP 集成（通过 acpx 调用编程 Agent）

MicroClaw 支持通过 [ACP (Agent Client Protocol)](https://agentclientprotocol.com/) 调用外部编程 Agent（Claude Code、Gemini CLI、Codex 等）。底层使用 [acpx](https://github.com/openclaw/acpx) 作为 headless ACP 客户端。

> **前置要求：** Node.js v20.10+（ACP 适配器依赖 import attributes 语法）
...
# 固定安装兼容版本的 Claude ACP 适配器（避免 npx 每次拉最新导致不兼容）
npm install -g @zed-industries/claude-agent-acp@0.19.2
```

### 3. 配置环境变量和模型

acpx 通过 ACP 适配器启动 Claude Code，**不会读取** `~/.claude/settings.json` 中的 `apiKeyHelper` 和 `env` 配置。需要在 shell 环境中导出：
...
建议写入 `~/.bashrc` 或 `~/.zshrc` 以持久化，或在 MicroClaw 的启动脚本/systemd 配置中设置。

**模型配置**：需要同时在环境变量和 `~/.claude/settings.json` 中配置模型，确保 ACP 适配器使用正确的模型版本。
...
常用模型 ID：
- 直连 Anthropic API：`claude-opus-4-6`、`claude-sonnet-4-6`
- AWS Bedrock：`us.anthropic.claude-opus-4-6-v1`、`us.anthropic.claude-sonnet-4-6-v1`

> **注意事项**：
> - `ANTHROPIC_MODEL` 环境变量**不能为空字符串**（会导致 "invalid model identifier" 错误），要么正确设置、要么完全不设置（`unset ANTHROPIC_MODEL`）
> - `CLAUDE_CODE_USE_BEDROCK=0` 在 JavaScript 中是 truthy（非空字符串），如果不使用 Bedrock 请用 `unset` 而不是设为 `0`
> - 适配器 v0.19.2 的模型匹配存在已知 bug：`getAvailableModels()` 中 `models.find()` 使用 `settings.model.includes(m.value)` 做子串匹配，会导致 `"opus"`（Opus 4.1）在列表中先于 `"us.anthropic.claude-opus-4-6-v1"`（Opus 4.6）被匹配到。修复方法见下方 patch 步骤

**修复适配器模型匹配 bug（可选）**：

如果即使配置了环境变量和 settings.json，acpx 仍然使用旧版模型，需要手动 patch 适配器，将精确匹配优先于模糊匹配：

```sh
ADAPTER_JS="$(npm root -g)/@zed-industries/claude-agent-acp/dist/acp-agent.js"

# 备份
sudo cp $ADAPTER_JS ${ADAPTER_JS}.bak

# 修改 getAvailableModels 函数：精确匹配优先
# 找到包含 "const match = models.find" 的行，将单次 find 改为先精确匹配再模糊匹配
sudo node -e "
const fs = require('fs');
const code = fs.readFileSync('$ADAPTER_JS', 'utf-8');
const patched = code.replace(
  /const match = models\.find\(\(m\) => m\.value === settings\.model \|\|[\s\S]*?m\.displayName\.toLowerCase\(\)\.includes\(settings\.model\.toLowerCase\(\)\)\);/,
  'const match = models.find((m) => m.value === settings.model) || models.find((m) => m.value.includes(settings.model) || m.displayName.toLowerCase() === settings.model.toLowerCase() || m.displayName.toLowerCase().includes(settings.model.toLowerCase()));'
);
if (code !== patched) { fs.writeFileSync('$ADAPTER_JS', patched); console.log('Patched'); }
else { console.log('Pattern not found, may need manual edit'); }
"
```

> 此 patch 在适配器升级后需要重新应用。建议关注 [claude-agent-acp](https://github.com/zed-industries/claude-agent-acp) 上游修复。

### 4. 验证 acpx + Claude Code
...
acpx claude -s test "你是什么模型"
```

如果 `sessions new` 没有输出 session ID，请检查：
- Claude Code 是否已安装并登录（`claude --version`）
- 环境变量是否正确导出（`echo $ANTHROPIC_API_KEY`）
- Node.js 版本是否 >= 20.10（`node --version`）

### 5. 配置 MicroClaw

在 `microclaw.config.yaml` 中添加：

```yaml
acp:
  enabled: true
  default_agent: "claude"      # claude / gemini / codex / opencode
  default_cwd: "/home/ubuntu/microclaw/"
  timeout: 300
  default_permission: "approve-reads"  # approve-reads / approve-all / deny-all
```

权限说明：
- `approve-reads` — Agent 可读取文件，写操作被拒绝（默认，安全）
- `approve-all` — Agent 可自由读写文件和执行命令
- `deny-all` — 仅对话，不允许任何文件操作

### 6. 使用

在任意渠道（Telegram / Discord / Web 等）对 MicroClaw 说：

```
帮我用 Claude Code 重构 src/main.rs 的错误处理
```

MicroClaw 的 Agent 引擎会自动选择 `acp_agent` 工具，通过 acpx 调用 Claude Code 执行任务并返回结果。

也支持指定其他 Agent 和参数：

```
用 gemini 分析一下项目的依赖结构
```
